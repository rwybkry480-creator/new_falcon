# -----------------------------------------------------------------------------
# smc_bot_v17.5.py - (Falcon KDJ Sniper v17.5: The Bleeding Edge)
# -----------------------------------------------------------------------------
...
@app.route('/')
def health_check():
    return "Falcon KDJ Sniper Bot Service (v17.5 - Bleeding Edge) is Running!", 200
def run_server():
    port = int(os.environ.get("PORT", 10000))
    app.run(host='0.0.0.0', port=port)

# --- دوال التحليل (النظر إلى الشمعة الأخيرة فقط) ---
...
def analyze_symbol_kdj(df):
    try:
        ...
        # --- *** المنطق المطلوب: ننظر فقط إلى آخر شمعة حالية (iloc[-1]) *** ---
        current_candle = df.iloc[-1]

        # الشرط: هل J هو القائد في الشمعة الحالية؟
...
async def scan_market(context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"--- Starting {job_name} (v17.5 - Bleeding Edge) ---")
    chat_id = context.job.data['chat_id']
    if job_name == "Manual Scan":
        await context.bot.send_message(chat_id=chat_id, text=f"⏳ جاري {job_name} للسوق (فريم 1 ساعة، الشمعة الحالية فقط v17.5)...")
    ...
    for symbol in symbols_to_scan:
        ...
        if signal_type == 'BUY':
            found_signals += 1
            message = (f"⚡️ *[KDJ 1h - Bleeding Edge]* حالة زخم إيجابي فوري!\n\n"
                       f"• **العملة:** `{symbol}`\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_html(f"👋 أهلاً بك يا {user.mention_html()}!\n\n"
                                    f"أنا بوت **Falcon KDJ Sniper (v17.5 - The Bleeding Edge)**.\n\n"
                                    f"أقوم بالبحث عن حالة `J > K` و `J > D` في **الشمعة الحالية** (الأخيرة).\n"
                                    f"سيتم إجراء فحص تلقائي كل ساعة.")
...
def run_bot():
    ...
    logger.info("--- [Falcon KDJ Sniper v17.5] Bot is ready and running autonomously. ---")
    application.run_polling()

if __name__ == "__main__":
    logger.info("--- [Falcon KDJ Sniper v17.5] Starting Main Application ---")
    server_thread = Thread(target=run_server)
    server_thread.daemon = True
    server_thread.start()
    logger.info("--- [Falcon KDJ Sniper v17.5] Web Server has been started. ---")
    run_bot()
